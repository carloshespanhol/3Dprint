...
module cable_mount(){
    difference(){
        rotate([90,0,0])
        translate([0,-279,-35])
        import("/home/hespanhol/3D Print/openscad/HC230/stl/Cable_Mount.stl");
        //-------------------------------
        translate([-12.5,-10,-1])
        cylinder(d=5.5,h=10);
        translate([12.5,-10,-1])
...
import("/home/hespanhol/OneDrive/openscad/HC235/EVA_stl/tension_slider_6mm_belt_M3s.stl");
    ...
    union(){
        ...
        translate([0,tbd+2.6+1.25,-belt_size-belt_distance/2])
        cube([dx,bt,6]);

        // Right
        translate([dx,tbd+2.6+1.25,belt_distance/2])
        cube([dx,bt,6]);
...
module gt2_idler(type="flat", b=3){ 
    // 12 x 3 x 6.5mm
    /**
    if(type=="flat"){
...
// Parts
module nema17_holes(sd=m3, h=10){
    // center hole
    pcylinder(d = 22.5, h = h);
...
module e3d(detaillevel=1, socket_width=0) {
	...
	difference() {
		union() {
            color("black")
			translate([0,0,2.8]) fncylinder(r=1,h=1);
            color("silver"){
			translate([0,0,2.9]) fncylinder(r=8,h=4);
