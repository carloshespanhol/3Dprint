...
*translate([0,(nut_m3[0]+stable_wall*2)/2+1,-5]) lcdbox_mount_rear();
*translate([0,-extrusion_width-5, extrusion_width/2+stable_wall*1.5]) rotate([90,0,180]) lcdbox_mount_back();


/* Mounted *
translate([0,0,-20]) %centercube([100, 20, 20],x=true,y=true);
lcdbox_mount_back();
translate([0,3.5,14]) rotate([90,0,0]) lcdbox_mount_rear();
translate([46.5,12.25 -extrusion_width/2-stable_wall,6.5]) rotate([90,0,180]) lcdbox_bottom();
%translate([-46.5,36.75 -extrusion_width/2-stable_wall, 6.5]) rotate([90,0,0]) lcdbox_top();
/**/
// #####################################################################################################################################
...
screw_m5		= [2.7,		4,	5,		"screw M5"];
nut_m3			= [5.5,		3.04,	2.4,		6,	"nut M3"];
washer_m5		= [5.5,		1,		2.7,	"washer M5"];

extrusion_width = 20;
extrusion_screw_r	 = screw_m5[0];
extrusion_screw_head_h	 = screw_m5[2];
extrusion_washer_r = washer_m5[0];
extrusion_washer_h = washer_m5[1];
extrusion_channel  = 5;
// #####################################################################################################################################

module lcdbox_mount_rear() {
	difference() {
		union() {
            translate([0, -4, 5])
			centercube([15+nut_m3[1]*2+2,stable_wall*6, stable_wall],x=true,y=false);
           
           hull(){ 
            translate([0, -4, 5])
			centercube([15+nut_m3[1]*2+2, (nut_m3[0]+stable_wall*2), stable_wall],x=true,y=false);
		...
		for(i1=[-1,1]) 
            translate([i1*7.5, 13,-1]) fncylinder(r=screw_m3[0]+compensate,h=stable_wall*4);
...
module lcdbox_mount_back() {
    w=15+nut_m3[1]*2+2+compensate*2+stable_wall*2;
	difference() {
		union() {
            // chanel fixer
            translate([-w/2, -extrusion_width/2, -extrusion_width/2-extrusion_channel/2])
            cube([6,5,extrusion_channel]);            
            translate([w/2-6, -extrusion_width/2, -extrusion_width/2-extrusion_channel/2])
            cube([6,5,extrusion_channel]);
            //
            translate([0,-extrusion_width/2-stable_wall*1.5, -extrusion_width])
				centercube([w, stable_wall*1.5, extrusion_width+15],x=true,y=false);
            
			hull() {
                translate([0,-extrusion_width/2-stable_wall/2, 2])
				centercube([w, stable_wall, stable_wall*2],x=true,y=true);
                
				translate([0, -extrusion_width/2, nut_m3[0]+stable_wall*2+1.5]) rotate([0,90,0])
		...
		translate([0, -extrusion_width/2, nut_m3[0]+stable_wall*2+1.5]) rotate([0,90,0])
           ...
           translate([-.1,-extrusion_width/2-stable_wall*3, -extrusion_width])
				centercube([w+1, stable_wall*1.5, extrusion_width*3],x=true,y=false);
        
		translate([0, -extrusion_width/2, nut_m3[0]+stable_wall*2+1.5]) rotate([0,90,0])
			fncylinder(r=(nut_m3[0]+stable_wall*2)/2+1,h=15+nut_m3[1]*2+2+compensate*2,center=true);
    
    // screw    
    translate([0, -extrusion_width/2, -extrusion_width/2]) rotate([90,0,0]) fncylinder(r=extrusion_screw_r+compensate,h=stable_wall,enlarge=1);

    translate([0, -extrusion_width/2-5, -extrusion_width/2]) rotate([90,0,0]) fncylinder(r=extrusion_screw_r*2+compensate,h=stable_wall*2,enlarge=1);

	}
    
    // sacrifice bridge
    translate([0, -extrusion_width/2-3.9, -extrusion_width/2]) rotate([90,0,0]) fncylinder(r=extrusion_screw_r*2+compensate,h=.2,enlarge=0);
...
module lcdbox_bottom() {
	difference() {
		union() {
			...
			intersection() {
				union() for(i1=[	[lcd_mountoffset_sides,lcd_mountoffset_bottom],
								...
								[lcd_mountoffset_sides,lcd_depth-lcd_mountoffset_sides],
								[lcd_width-lcd_mountoffset_sides,lcd_depth-lcd_mountoffset_sides]]) 
								translate(i1) fncylinder(r=lcd_mountholes_head_r+compensate+normal_wall,h=lcdbox_bottommount_hight);
				translate([-compensate-small_wall,-compensate-small_wall,0])
					centercube([lcd_width+compensate*2+small_wall*2,lcd_depth+compensate*2+small_wall*2,lcdbox_bottom_hight]);
			}
		...
		// backmount holes
		for(i1=[-1,1]) translate([lcd_width/2+i1*lcd_backmount_holes_offset,lcd_mountoffset_bottom+1,0]) fncylinder(r=screw_m3[0]+compensate,h=stable_wall,enlarge=1);
	}
    ...
    for(i1=[	[lcd_mountoffset_sides,lcd_mountoffset_bottom],
                ...
                [lcd_width-lcd_mountoffset_sides,lcd_depth-lcd_mountoffset_sides]])
    translate(i1) {
        translate([0,0,lcd_mountholes_head_h]) 
        fncylinder(r=lcd_mountholes_head_r+compensate,h=.2);
    }
	...
	translate(center==false?[0,0,-enlarge]:[0,0,-h/2-enlarge]) {
		if (fn==undef) {
			if (r2==undef && d2==undef) {
				...
				cylinder(r1=r?r:d/2,r2=r2?r2:d2/2,h=h+enlarge*2,$fn=floor(2*(r?r:d/2)*pi/fnr));
			}
		} else {
			if (r2==undef && d2==undef) {
