...
if(print==1){
    ...
    *color("orange") 
    ...
    //}
    /* Idlers */
    translate([fix,16.75-14, belt_distance/2-1]) 
    %f623zz_idler();
...
module xy_joiner(side="L"){   
    ...
    difference(){      
        union(){  
            /* body */
            difference(){      
                translate([ad/2,-w/2+10,-16.5])
                rcube(xs,w, ph+10, 4);
                
                translate([ad/2+bw/2+3,-2,bbi])
                cube([xs,25,ph]);
            }
            //
     *       translate([ad/2+xs-17.5, -w/2+10,-15.5])
            rcube(17.5, 19-5, ph+8, 4);
        ...
        //-------------------------------
        translate([fix/2+8.5,16.75-14, belt_distance/2-2]) 
        cylinder(d=13,h=10);

        translate([fix/2-10,-16.75-5, belt_distance/2-2]) 
        cube([14.5,30,9]);
        
        /* bush */
        translate([4,-ybush_len/2+9.5,0])
        rotate([-90,0,0])
        cylinder(d=bw, h=ybush_len+1);
        
        translate([4,-ybush_len/2,0])
        rotate([-90,0,0])
        cylinder(d=10.5, h=ybush_len+20);

        // X Axis
        translate([49-20,-7-1,0])
        rotate([0, 90, 0]) 
        rotate([0, 0, 30]) 
        rpol(d=8+pressfit,h=220);   

        /* Bush fixing*/
        translate([ad/2,w/3-3+10,0])
        rotate([-90,0,0])
        ring(12,10,4);
        translate([ad/2,-w/3-1+10,0])
        rotate([-90,0,0])
        ring(12,10,4);
