...
*hull(){
    ...
    cube([390+80+f,by+10+f/2,z+130+f/2]);
...
module zBottom(){
    // bridge
    #translate([40,-20, 15-2.2])
    cylinder(d=22+fit, h=.2);

    color(c3dp)
    difference(){
        hull(){
            translate([-10,-36,0])
            rcube(sd+30,35,15,16);
...
module zBack(){
    // T8 nuts
   *translate([61,34,zh/2+15])
    rotate([0,-90,0])
    import("stl/TR8x2-4_Nut.stl");
   *translate([18,34,zh/2+15])
    rotate([0,90,0])
    import("stl/TR8x2-4_Nut.stl");
    ...
    difference(){
        union(){
            ...
            translate([0,0,15])
            cube([10-fit,20,zh-fit]);

            translate([70+fit,0,15])
            cube([10-fit,20,zh-fit]);
        }
        
        //----------------------
        // tnut space
        translate([13, 28, dz/2+16])
        cube([54,10,38]);
        
        translate([13-.1, 34, dz/2+35])
        rotate([0,90,0])
        cylinder(d=23, h=13.1);
        translate([58-4, 34, dz/2+35])
        rotate([0,90,0])
        cylinder(d=23, h=30);

        translate([-.1, 36, dz/2+35])
        rotate([0,90,0])
        cylinder(d=32, h=13.1);
        translate([67, 36, dz/2+35])
        rotate([0,90,0])
        cylinder(d=32, h=20);

        // tnut bolts
        translate([39.5,30-5-2,79.5])
        rotate([90,0,0]){
            translate([0,0,0])
            Mscrew(20,5.3);
            translate([0,21,0])
